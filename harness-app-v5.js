...
const RUNNERS = {
  ...
  server: {
    label: '내부 서버', metric: 'SERVER', endpoint: '', connected: false,
...
function refreshRunButton() {
  ...
  runButton.disabled = isRunning || !files.length || !validRequest || !RUNNERS[runnerMode].connected;
  $('#runHint').textContent = isRunning ? '문서를 처리하고 있습니다.' : !files.length ? '문서를 선택하면 검토를 시작할 수 있습니다.' : !validRequest ? '업무 요청을 10자 이상 입력해 주세요.' : !RUNNERS[runnerMode].connected ? '실행기 연결을 먼저 확인해 주세요.' : '선택한 문서로 검토를 시작할 수 있습니다.';
  runButton.innerHTML = `${isRunning ? '검토 중…' : ({browser:'브라우저에서 검토 시작',agent:'PC 에이전트에서 검토 시작',server:'내부 서버에서 검토 시작'})[runnerMode]} <span>→</span>`;
}

function renderFileList() {
  const total = files.reduce((sum, file) => sum + file.size, 0);
  $('#fileSummary').textContent = files.length ? `${files.length}개 문서 · ${formatBytes(total)}` : '선택된 문서 없음';
  $('#fileList').innerHTML = files.map((file, index) => `<span>${escapeHtml(file.name)} <small>${formatBytes(file.size)}</small><button type="button" data-remove-file="${index}" aria-label="${escapeHtml(file.name)} 선택 취소">×</button></span>`).join('');
  refreshRunButton();
}
...
function setConnectionVisual(status, title, note) {
  ...
  $('#securityConnection').textContent = status === 'ready' ? '연결됨' : status === 'checking' ? '확인 중' : '연결 필요';
...
function applyRunner(mode) {
  ...
  runnerMode = mode;
  document.body.dataset.runner = mode;
  ...
  $('#agentInstall').hidden = mode !== 'agent';
  ...
  if (mode === 'browser') {
    ...
    $('#securityProcessing').textContent = '현재 브라우저';
    $('#securityEgress').textContent = '0 B';
    $('#securityHasa').textContent = '사용하지 않음';
    $('#securityKey').textContent = '필요 없음';
    $('#sealTitle').textContent = 'BROWSER LOCAL';
    $('#sealNote').textContent = '선택한 문서만 이 탭에서 처리합니다.';
    $('#egressPill').innerHTML = '<span class="live-dot"></span>원문 외부 송신 · 0 B';
    ...
    $('#securityProcessing').textContent = mode === 'agent' ? '이 PC' : '내부 서버';
    $('#securityEgress').textContent = mode === 'agent' ? '이 PC 안에서 처리' : '선택한 내부 서버';
    $('#securityHasa').textContent = '마스킹한 요청·파일명·근거';
    $('#securityKey').textContent = 'PC 또는 내부 실행기';
    $('#sealTitle').textContent = mode === 'agent' ? 'PC AGENT' : '내부 서버';
    $('#sealNote').textContent = 'HASA 키는 웹이 아닌 실행기 환경변수에만 둡니다.';
    $('#egressPill').innerHTML = '<span class="live-dot"></span>HASA · 마스킹한 요청·파일명·근거 ONLY';
...
async function connectRunner(options = {}) {
  ...
  try {
    ...
    if (revision !== connectionRevision || runnerMode !== modeAtStart) return;
...
function selectDocuments(selected) {
  if (selected.length > MAX_FILES) showToast(`최대 ${MAX_FILES}개 문서만 선택할 수 있습니다.`);
...
documentInput.addEventListener('change', () => selectDocuments([...documentInput.files]));
$('#documentDropZone').addEventListener('dragover', event => { event.preventDefault(); event.currentTarget.classList.add('is-dragging'); });
$('#documentDropZone').addEventListener('dragleave', event => event.currentTarget.classList.remove('is-dragging'));
$('#documentDropZone').addEventListener('drop', event => { event.preventDefault(); event.currentTarget.classList.remove('is-dragging'); if (!isRunning) selectDocuments([...event.dataTransfer.files]); });
$('#fileList').addEventListener('click', event => { const button=event.target.closest('[data-remove-file]'); if(button && !isRunning){ files.splice(Number(button.dataset.removeFile),1); documentInput.value=''; renderFileList(); } });
...
$('#runnerEndpoint').addEventListener('input', invalidateConnection);
$('#runnerToken').addEventListener('input', invalidateConnection);
...
async function executeRemote(request, details) {
  ...
  const response = await fetch(`${endpoint}/v1/runs`, {
    method: 'POST', headers: runnerHeaders(true), body: JSON.stringify({ request, files: payloadFiles }), signal: AbortSignal.timeout(120000)
  });
...
async function executeRun() {
  ...
  isRunning = true;
  refreshRunButton();
  $('#activeTitle').textContent = '선택한 실행기에서 처리 중입니다.';
  $('#activeDescription').textContent = '문서를 읽고 검토 근거를 생성합니다.';
  $('#runStatus').textContent = '처리 중';
  ...
  $('#planOutput').textContent = JSON.stringify({
    ...
    hasa_input: runnerMode === 'browser' ? 'none' : 'pattern-masked request, filenames and evidence',
    output: 'markdown'
  }, null, 2);
  $('#policyCheck').textContent = runnerMode === 'server' ? '✓ 원문은 선택한 내부 서버로 전달됩니다.' : '✓ 원문은 이 기기에서 처리합니다.';
  ...
  renderResult(result);
  renderSteps(5, details);
  $('#activeTitle').textContent = '완료된 실행 내역';
  $('#activeDescription').textContent = '이번 작업에서 처리한 문서와 실행 단계를 확인하세요.';
...
function renderResult(result) {
  ...
  $('#resultPrivacyCheck').textContent = hasaUsed ? '✓ HASA 입력: 마스킹한 요청·파일명·근거' : '✓ HASA 전송 0 B';
...
navItems.forEach((item) => item.addEventListener('click', () => {
  ...
  if (isRunning && next !== 'active') return showToast('검토가 끝난 뒤 이동할 수 있습니다.');
...
$$('[data-action]').forEach((item) => item.addEventListener('click', () => {
  if (isRunning) return showToast('검토가 끝난 뒤 이동할 수 있습니다.');
  if (item.dataset.action === 'documents') {
    setState('ready');
    documentInput.click();
  } else if (item.dataset.action === 'install') {
    setState('ready');
...
$('#copyInstallCommand').addEventListener('click', async () => {
  ...
  try {
    ...
    showToast('설치 명령어를 복사했습니다.');
  } catch (_) {
    showToast('명령어를 선택해 직접 복사해 주세요.');
  }
});

$('#resetButton').addEventListener('click', () => { if(isRunning) return showToast('검토가 끝난 뒤 새 작업을 시작할 수 있습니다.'); files=[]; documentInput.value=''; lastResult=null; renderFileList(); setState('ready'); taskInput.focus(); });
