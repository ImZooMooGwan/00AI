...
export default function ApiPage() {
  ...
  return <SubpageFrame eyebrow="OPEN API · VERSION 1" title="Open API v1" description="정책·변경·지표·출처를 같은 ID와 데이터 버전으로 조회합니다. 모든 응답에 생성시점·건수·출처 수·라이선스를 포함합니다." aside={<dl><div><dt>API 버전</dt><dd>v1</dd></div><div><dt>데이터셋</dt><dd>{snapshot.datasetVersion}</dd></div><div><dt>응답 형식</dt><dd>JSON</dd></div><div><dt>OpenAPI</dt><dd>3.1.0</dd></div></dl>}><div className="api-console"><header><div><span className="api-light red" /><span className="api-light amber" /><span className="api-light green" /></div><code>https://y-hub.example/api/v1</code><a href="/api/openapi">OpenAPI JSON ↗</a></header><pre>{JSON.stringify(example, null, 2)}</pre></div><div className="endpoint-list">{endpoints.map(([method,path,description]) => <a href={path.replace("{id}","YH-POL-0001")} key={path}><b>{method}</b><code>{path}</code><span>{description}</span><i>↗</i></a>)}</div><div className="api-params"><h2>공통 쿼리</h2>{["as_of","from","to","region","category","status","verification_status","changed_since","page","page_size","format"].map((param) => <code key={param}>{param}</code>)}</div></SubpageFrame>;
}
