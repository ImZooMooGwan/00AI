...
- HASA를 사용할 때는 이메일·휴대전화·주민등록번호 패턴을 가린 업무 요청·파일명·근거 문장을 전송합니다.
...
4. HASA 키가 있으면 업무 요청·파일명·근거 문장의 이메일·휴대전화·주민등록번호 패턴을 가린 뒤 HASA에 전송합니다. 원본 전체와 API Key는 웹 Harness로 보내지 않습니다.
5. 실행기는 결과와 근거를 웹 Harness에 반환하고, 이용자가 Markdown 결과를 내려받습니다.

PC 모드는 `127.0.0.1`에만 바인딩하고 허용 Origin을 제한합니다. 기관 내부 서버 모드는 반드시 HTTPS 리버스 프록시, `HARNESS_AGENT_TOKEN`, 방화벽·이용자 접근통제와 함께 사용해야 합니다. 현재 패턴 마스킹은 이름·주소 등 모든 개인정보를 탐지하지 않습니다. 기관의 반출 심사와 전송 통제를 대체하지 않습니다.
